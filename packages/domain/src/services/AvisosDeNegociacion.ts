 ...
 */
export class AvisosDeNegociacion {
    constructor(private readonly notifier: INotifier) {}

    async ofertaRecibida(operation: Operation): Promise<void> {
    ...
    async contraofertaHecha(operation: Operation): Promise<void> {
        const { props } = operation.toSnapshot();
        const destinatario =
    ...
    async ofertaAceptada(operation: Operation, aceptoLa: NegotiatingParty): Promise<void> {
        const { props } = operation.toSnapshot();
        const otraParte = aceptoLa === 'buyer' ? props.sellerId : props.buyerId;
        const precio = operation.finalPrice ?? operation.currentOfferPrice;
        ...
        await this.enviar([
            Notification.create({
                ...
                operationId: operation.id,
                listingId: props.listingId,
                amountCents: precio.getCents(),
                currency: precio.getCurrency(),
            }),
        ]);
    }
     ...
     */
    async ofertasCanceladasPorCascada(canceladas: Operation[]): Promise<void> {
    ...
    async listingRevisado(listing: Listing, aprobado: boolean): Promise<void> {
    ...
    async contratoFirmado(operation: Operation): Promise<void> {
        await this.aAmbasPartes(operation, 'contrato_firmado');
    }

    /** El activo llegó a custodia: al comprador le toca pagar. */
    async activoEnCustodia(operation: Operation): Promise<void> {
        const { props } = operation.toSnapshot();
    ...
    async pagoConfirmado(operation: Operation): Promise<void> {
        const { props } = operation.toSnapshot();
    ...
    async operacionCompletada(operation: Operation): Promise<void> {
        await this.aAmbasPartes(operation, 'operacion_completada');
    }

    // ── Interno ──────────────────────────────────────────

    private async aAmbasPartes(
        operation: Operation,
    ...
    private async enviar(notificaciones: Notification[]): Promise<void> {
        try {
            await this.notifier.notificar(notificaciones);
        } catch {
