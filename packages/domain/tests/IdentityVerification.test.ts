...
describe('User.verifyIdentity', () => {
    ...
    it('presentar el documento lo deja verificado', () => {
        ...
        user.verifyIdentity({ dni: '20123456', country: 'AR' });
    ...
    it('acepta un documento con puntos y guiones', () => {
        ...
        user.verifyIdentity({ dni: '20.123.456' });

        expect(user.isKycVerified).toBe(true);
    });

    it('rechaza un documento vacío', () => {
        expect(() => unUsuarioNuevo().verifyIdentity({ dni: '   ' }))
            .toThrow(ValidationError);
    });

    it('rechaza un documento con letras', () => {
        expect(() => unUsuarioNuevo().verifyIdentity({ dni: 'ABC12345' }))
            .toThrow(ValidationError);
    });

    it('rechaza un documento demasiado corto', () => {
        expect(() => unUsuarioNuevo().verifyIdentity({ dni: '123' }))
            .toThrow(ValidationError);
    });
    ...
    it('no deja verificar dos veces', () => {
        ...
        expect(() => user.verifyIdentity({ dni: '20123456' }))
    ...
    it('guarda el teléfono y el país cuando se los pasan', () => {
        const user = unUsuarioNuevo();
        user.verifyIdentity({ dni: '20123456', phone: '+541155550000', country: 'AR' });
