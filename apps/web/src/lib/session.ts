 ...
 */
export async function saveSession(token: string, actor: ActorDto): Promise<void> {
...
export async function readSession(): Promise<{ token: string; actor: ActorDto } | null> {
    const almacen = await cookies();
...
export async function currentActor(): Promise<ActorDto | null> {
    return (await readSession())?.actor ?? null;
}
